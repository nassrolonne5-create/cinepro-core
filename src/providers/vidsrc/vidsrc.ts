...
export class VidSrcProvider extends BaseProvider {
    ...
    async getMovieSources(media: ProviderMediaObject): Promise<ProviderResult> {
        return {
            sources: [
                {
                    url: this.createProxyUrl(`https://vidsrc.to/embed/movie/${media.tmdbId}`, this.HEADERS),
                    quality: 'auto',
                    type: 'embed',
                    audioTracks: [],
                    provider: {
    ...
    async getTVSources(media: ProviderMediaObject): Promise<ProviderResult> {
        ...
        return {
            sources: [
                {
                    url: this.createProxyUrl(`https://vidsrc.to/embed/tv/${media.tmdbId}/${media.s}/${media.e}`, this.HEADERS),
                    quality: 'auto',
                    type: 'embed',
                    audioTracks: [],
                    provider: {
