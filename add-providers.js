...
const providers = ['purstream', 'peachify', 'vidcore', 'trendimovies', 'vidfast', 'mapple', 'lmscript'];

providers.forEach(p => {
    const dir = `src/providers/${p}`;
    if (!fs.existsSync(dir)) fs.mkdirSync(dir, { recursive: true });
    
    const Name = p.charAt(0).toUpperCase() + p.slice(1);
    
    const content = `import { BaseProvider } from '@omss/framework';
import type { ProviderCapabilities, ProviderMediaObject, ProviderResult, Source } from '@omss/framework';
import { fetchSources } from 'kaizoku-core/providers/movies/${p}';
...
`;
    fs.writeFileSync(path.join(dir, `${p}.ts`), content);
    console.log(`Added ${p}`);
});
